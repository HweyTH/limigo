...
type FixedWindowStore interface {
	// Allow increments the request count for key within the current window and
...
type SlidingWindowStore interface {
	// Allow records the current request for key and returns true if the number of
...
type TokenBucketStore interface {
	// Allow attempts to consume one token from the bucket for key, refilling based
	...
	Allow(ctx context.Context, key string, capacity float64, rate float64) (bool, error)
}
