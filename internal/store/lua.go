...
// window, and token bucket algorithms, plus their batched delta-sync variants
// used by node-local caching, embedded into the binary at build time so
// RedisStore has no runtime dependency on the working directory.
func LoadEmbeddedScripts() (fixedWindow, slidingWindow, tokenBucket, fixedWindowSync, tokenBucketSync string, err error) {
	...
	return string(fxScript), string(swScript), string(tbScript), string(fxSyncScript), string(tbSyncScript), nil
}
